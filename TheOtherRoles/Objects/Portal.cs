...
namespace TheOtherRoles.Objects
{
    ...
    public class Portal
    {
        ...
        public static void startTeleport(byte playerId, byte exit)
        {
            ...
            string playerNameDisplay = Portalmaker.logOnlyHasColors ? string.Format("portalLogOnlyHasColors".Translate(), (Helpers.isLighterColor(playerControl) ? "colorLightMax".Translate() : "colorDarkMax".Translate())) : playerControl.Data.PlayerName;

            int colorId = playerControl.Data.DefaultOutfit.ColorId;

            if (Camouflager.camouflageTimer > 0 || Helpers.MushroomSabotageActive())
            {
                playerNameDisplay = ModTranslation.getString("playerNameDisplay");
                colorId = 6;
            }
