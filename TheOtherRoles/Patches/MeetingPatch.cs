    ...
    class MeetingHudPatch
    {
            ...
            private static Dictionary<byte, int> CalculateVotes(MeetingHud __instance)
            {
                ...
                for (int i = 0; i < __instance.playerStates.Length; i++)
                {
                    ...
                    if (playerVoteArea.VotedFor != 252 && playerVoteArea.VotedFor != 255 && playerVoteArea.VotedFor != 254)
                    {
                        ...
                        int currentVotes;
                        int additionalVotes = (Mayor.exists && Mayor.players.Any(x => x.player.PlayerId == playerVoteArea.TargetPlayerId) && Mayor.voteTwice) ? 2 : 1; // Mayor vote
        ...
        class MeetingHudBloopAVoteIconPatch
        {
            public static bool Prefix(MeetingHud __instance, NetworkedPlayerInfo voterPlayer, int index, Transform parent)
            {
                ...
                var showVoteColors = !GameManager.Instance.LogicOptions.GetAnonymousVotes() ||
                                      ...
                                      (PlayerControl.LocalPlayer.isRole(RoleId.Mayor) && Mayor.canSeeVoteColors && TasksHandler.taskInfo(PlayerControl.LocalPlayer.Data).Item1 >= Mayor.tasksNeededToSeeVoteColors);
                if (showVoteColors)
                {
            ...
            private static bool Prefix(MeetingHud __instance, Il2CppStructArray<MeetingHud.VoterState> states)
            {
                ...
                for (int i = 0; i < __instance.playerStates.Length; i++)
                {
                    ...
                    for (int j = 0; j < states.Length; j++)
                    {
                        ...
                        if (Mayor.exists && Mayor.players.Any(x => x.player.PlayerId == voterState.VoterId) && !mayorFirstVoteDisplayed && Mayor.voteTwice)
                        {
        ...
        private static void swapperOnClick(int i, MeetingHud __instance)
        {
            ...
            else if (selectedCount == 1)
            {
                ...
                else
                {
                    selections[i] = true;
                    renderer.color = Color.yellow;
                    meetingExtraButtonLabel.text = Helpers.ColorString(Color.yellow, "meetingSwapperButtonLabel".Translate());
                }
            }
            else if (selectedCount == 2)
            {
                if (selections[i])
                {
                    renderer.color = Color.red;
                    selections[i] = false;
                    meetingExtraButtonLabel.text = Helpers.ColorString(Color.red, "meetingSwapperButtonLabel".Translate());
                }
            }
        }
        ...
        static void swapperConfirm(MeetingHud __instance)
        {
            ...
            if (firstPlayer != null && secondPlayer != null)
            {
                ...
                meetingExtraButtonLabel.text = Helpers.ColorString(Color.green, "meetingSwappingText".Translate());
                Swapper.charges--;
        ...
        public static void swapperCheckAndReturnSwap(MeetingHud __instance, byte dyingPlayerId)
        {
            ...
            meetingExtraButtonText.text = $"{"meetingSwapsText".Translate()} {Swapper.charges}";
            meetingExtraButtonLabel.text = Helpers.ColorString(Color.red, "meetingSwapperButtonLabel");
        ...
        static void mayorToggleVoteTwice(MeetingHud __instance)
        {
            ...
            meetingExtraButtonLabel.text = Helpers.ColorString(Mayor.color,
                "mayorToggleVoteTwice".Translate() + (Mayor.voteTwice ? Helpers.ColorString(Color.green, "optionOn".Translate()) : Helpers.ColorString(Color.red, "optionOff".Translate())));
        }
            ...
            foreach (RoleInfo roleInfo in RoleInfo.allRoleInfos)
            {
                ...
                if (!PlayerControl.LocalPlayer.Data.IsDead && !Helpers.playerById((byte)__instance.playerStates[buttonTarget].TargetPlayerId).Data.IsDead) button.GetComponent<PassiveButton>().OnClick.AddListener((System.Action)(() =>
                {
                    ...
                    else
                    {
                        ...
                        if (!HandleGuesser.killsThroughShield && Medic.IsShielded(focusedTarget))
            ...
            if (addSwapperButtons || addMayorButton)
            {
                ...
                else if (addMayorButton)
                {
                    ...
                    meetingExtraButtonLabel.text = Helpers.ColorString(Mayor.color,
                        ...
                        (Mayor.voteTwice ? Helpers.ColorString(Color.green, "optionOn".Translate()) : Helpers.ColorString(Color.red, "optionOff".Translate())));
                }
        ...
        class StartMeetingPatch
        {
            public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo meetingTarget)
            {
                ...
                Role.allRoles.DoIf(x => x.player == __instance, x => x.OnMeetingStart());
                ...
                if (PlayerControl.LocalPlayer.isRole(RoleId.Portalmaker) || Helpers.shouldShowGhostInfo() &&
