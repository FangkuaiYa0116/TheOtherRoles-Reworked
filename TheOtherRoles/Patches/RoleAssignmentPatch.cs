...
namespace TheOtherRoles.Patches
{
    ...
    class RoleManagerSelectRolesPatch
    {
        ...
        private static void assignRoles()
        {
            ...
            //setRolesAgain();
            if (RoleDraft.isEnabled && CustomOptionHolder.draftModeChooseRoleNoTeam.getBool())
                foreach (var roleAssignment in playerRoleMap)
                {
                    setRoleDirectly(roleAssignment.Item1, roleAssignment.Item2);
                }
        }

        private static void setRoleDirectly(byte playerId, byte roleId)
        {
            // 直接设置角色，不检查阵营
