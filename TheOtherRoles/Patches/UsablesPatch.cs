    ...
    internal class EmergencyMinigameUpdatePatch
    {
        private static void Postfix(EmergencyMinigame __instance)
        {
            ...
            if (__instance.state == 1)
            {
                ...
                var remaining = Mathf.Min(localRemaining, PlayerControl.LocalPlayer.isRole(RoleId.Mayor) ? 1 : teamRemaining);
    ...
    class VitalsMinigamePatch
    {
        ...
        class VitalsMinigameUpdatePatch
        {
            ...
            static void Postfix(VitalsMinigame __instance)
            {
                ...
                if (PlayerControl.LocalPlayer.isRole(RoleId.Hacker) && Hacker.players.Any(x => x.player == PlayerControl.LocalPlayer && x.hackerTimer > 0f))
                {
                    for (int k = 0; k < __instance.vitals.Length; k++)
                    {
                        ...
                        if (vitalsPanel.IsDead)
                        {
                            ...
                            if (deadPlayer != null && k < hackerTexts.Count && hackerTexts[k] != null)
                            {
                                ...
                                hackerTexts[k].text = Math.Round(timeSinceDeath / 1000) + ModTranslation.getString("secondText");
                            }
                        }
                    }
                }
    ...
    class AdminPanelPatch
    {
        ...
        class CounterAreaUpdateCountPatch
        {
            ...
            static void Postfix(CounterArea __instance)
            {
                ...
                bool showHackerInfo = PlayerControl.LocalPlayer.isRole(RoleId.Hacker) && Hacker.players.Any(x => x.player == PlayerControl.LocalPlayer && x.hackerTimer > 0f);
