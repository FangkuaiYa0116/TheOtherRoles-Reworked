...
namespace TheOtherRoles.Patches
{
    ...
    internal class BodyReportPatch
    {
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo target)
        {
            ...
            bool isMedicReport = PlayerControl.LocalPlayer.isRole(RoleId.Medic) && Medic.players.Any(x => x.player.PlayerId == __instance.PlayerId);
    ...
    public static class MurderPlayerPatch
    {
        ...
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
        {
            ...
            {
                if (role.player == __instance) role.OnKill(target);
            }

            foreach (var role in new List<Role>(Role.allRoles))
            {
                if (role.player == target) role.OnDeath(__instance);
            }
