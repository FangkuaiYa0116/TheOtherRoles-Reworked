...
using UnityEngine;
using static TheOtherRoles.Modules.RoleDraft;
    ...
    class IntroPatch
    {
        ...
        public static IEnumerator CoBegin(IntroCutscene __instance)
        {
            ...
            if (GameManager.Instance.IsNormal())
            {
                ...
                if (PlayerControl.LocalPlayer.Data.Role.IsImpostor || (RoleDraft.isEnabled && CustomOptionHolder.draftModeChooseRoleNoTeam.getBool()))
                {
                ...
                yield return CoSelectRoles(__instance).WrapToIl2Cpp();
        ...
        public static void setupIntroTeam(IntroCutscene __instance, Il2CppSystem.Collections.Generic.List<PlayerControl> yourTeam)
        {
            ...
            {
                if (RoleDraft.isEnabled && CustomOptionHolder.neutralRolesCountMax.getSelection() > 0 && !CustomOptionHolder.draftModeChooseRoleNoTeam.getBool())
                    __instance.TeamTitle.text = $"<size=60%>{"crewmate".Translate()}" + Helpers.ColorString(Color.white, " / ") +
                                                Helpers.ColorString(neutralColor, "introNeutral".Translate()) + "</size>";
                return;
            }
            ...
            static public void SetRoleTexts(IntroCutscene._ShowRole_d__41 __instance)
            {
                ...
                if (modifierInfo != null)
                {
                    if (modifierInfo.roleId != RoleId.Lover)
                        __instance.__4__this.RoleBlurbText.text += Helpers.ColorString(modifierInfo.color, $"\n{modifierInfo.introDescription}");
                    else
                    {
                        var otherLover = PlayerControl.LocalPlayer == Lovers.lover1 ? Lovers.lover2 : Lovers.lover1;
                        __instance.__4__this.RoleBlurbText.text += Helpers.ColorString(Lovers.color,
                ...
                if (Deputy.knowsSheriff && Deputy.exists && Sheriff.exists)
                {
                    if (infos.Any(info => info.roleId == RoleId.Sheriff))
                    {
                        ...
                        if (deputy != null) __instance.__4__this.RoleBlurbText.text += Helpers.ColorString(Sheriff.color,
                        string.Format("sheriffRoleBlurbText".Translate(), deputy?.player?.Data?.PlayerName ?? ""));
                    }
                    else if (infos.Any(info => info.roleId == RoleId.Deputy))
                        __instance.__4__this.RoleBlurbText.text += Helpers.ColorString(Sheriff.color,
                        string.Format("deputyRoleBlurbText".Translate(), Deputy.getRole(PlayerControl.LocalPlayer)?.sheriff?.player?.Data?.PlayerName ?? ""));
                }
            }
