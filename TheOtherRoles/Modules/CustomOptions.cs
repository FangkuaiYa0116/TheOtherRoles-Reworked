...
namespace TheOtherRoles
{
    public class CustomOption
    {
        ...
        {
            return ModTranslation.getString(headerBGColor == default ? name : Helpers.ColorString(headerBGColor, name));
        }
        ...
        public void updateSelection(int newSelection, bool notifyUsers = true)
        {
            ...
            {
                DestroyableSingleton<HudManager>.Instance.Notifier.AddSettingsChangeMessage(CustomStringName.CreateAndRegister(getMessageName()), selections[newSelection].ToString(), false);
                try
                {
            ...
            try
            {
                ...
                if (TheOtherRolesPlugin.Version > versionInfo && versionInfo < Version.Parse("4.6.0"))
                {
                    ...
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer,
                        ModTranslation.getString("pastingVanillaFailed"));
                }
                else
                {
    ...
    class LobbyViewSettingsPatch
    {
        ...
        public static void createCustomButton(LobbyViewSettingsPane __instance, int targetMenu, string buttonName, string buttonText, CustomOptionType optionType)
        {
            ...
            if (torSettingsButton == null)
            {
                ...
                __instance.StartCoroutine(Effects.Lerp(2f, new Action<float>(p => { torSettingsButton.transform.FindChild("FontPlacer").GetComponentInChildren<TextMeshPro>().text = buttonText.Translate(); })));
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType)
        {
            ...
            foreach (var option in relevantOptions)
            {
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type)
                {
                    ...
                    if (option.headerBGColor != default)
                    {
                        categoryHeaderMasked.Background.GetComponent<SpriteRenderer>().color = option.headerBGColor;
                        categoryHeaderMasked.Divider.GetComponent<SpriteRenderer>().color = option.headerBGColor;
                    }
                ...
                viewSettingsInfoPanel.titleText.text = option.headerBGColor == default ? settingTuple.Item1 : Helpers.ColorString(option.headerBGColor, settingTuple.Item1);
                ...
                {
                    viewSettingsInfoPanel.titleText.text = "spawnChance".Translate();
                }
                if ((int)optionType == 99)
                {
    ...
    class GameOptionsMenuStartPatch
    {
        ...
        private static void removeVanillaTabs(GameSettingMenu __instance)
        {
            ...
            GameObject.Find("RoleSettingsButton")?.GetComponent<PassiveButton>().OnClick.AddListener((System.Action)(() =>
            {
                ...
                allButtonBackground.GetComponent<PassiveButton>().transform.GetChild(2).gameObject.GetComponent<SpriteRenderer>().sprite = Helpers.loadSpriteFromResources("TheOtherRoles.Resources.AllButtonBackground.png", 100f);
            ...
            if (torSettingsButton == null)
            {
                ...
                if (!isMainButton && buttonColor != default)
                {
                    __instance.StartCoroutine(Effects.Lerp(2f, new Action<float>(p =>
                    {
                        ...
                        torSettingsButton.GetComponent<PassiveButton>().transform.GetChild(3).gameObject.GetComponent<TextMeshPro>().text = buttonText.Translate();
                    })));
                }
                var torSettingsPassiveButton = torSettingsButton.GetComponent<PassiveButton>();
                torSettingsPassiveButton.OnClick.RemoveAllListeners();
        ...
        private static void createSettingButtons(GameSettingMenu __instance)
        {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic)
            {
                ...
                {
                    createCustomButton(__instance, next++, "GuesserSettings", "GuesserSettingsText", Color.yellow);
                }

                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", "ImpostorSettingsText", Palette.ImpostorRed);

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", "NeutralSettingsText", Color.gray);
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", "CrewmateSettingsText", Palette.CrewmateBlue);
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", "ModifierSettingsText", Color.yellow);
            }
            else if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek)
            {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", "HideNSeekMain", Color.blue);
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", "HideNSeekRoles", Color.blue);
            }
            else if (TORMapOptions.gameMode == CustomGamemodes.PropHunt)
            {
                createCustomButton(__instance, next++, "PropHunt", "PropHuntSetting", Color.yellow);
            }
        }
    ...
    class LegacyGameOptionsPatch
    {
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly)
        {
            ...
            foreach (var option in options)
            {
                ...
                {
                    if (option.stringId == "deputySpawnRate") //Deputy
                        sb.AppendLine($"- {Helpers.ColorString(Deputy.color, "Deputy")}: {option.getString()}");
                    else if (option.stringId == "jackalCanCreateSidekick") //Sidekick
                        sb.AppendLine($"- {Helpers.ColorString(Sidekick.color, "Sidekick")}: {option.getString()}");
                    else if (option.stringId == "lawyerIsProsecutorChance") //Prosecutor
                        sb.AppendLine($"- {Helpers.ColorString(Lawyer.color, "Prosecutor")}: {option.getString()}");
                }
            }
            ...
            foreach (CustomOption option in options)
            {
                ...
                if (option.parent != null)
                {
                    ...
                    sb.AppendLine(Helpers.ColorString(c, $"{option.getName()}: {option.getString()}"));
                }
                else
                {
                    if (option == CustomOptionHolder.crewmateRolesCountMin)
