...
internal class RoleDraft
{
        ...
        while (pickOrder.Count > 0)
        {
            ...
            while (timer < maxTimer || !picked)
            {
                ...
                if (PlayerControl.LocalPlayer.PlayerId == pickOrder[0])
                {
                    ...
                    playerText = Helpers.ColorString(youColor, "youText".Translate() + "!");
                    // Available Roles:
                    List<RoleInfo> availableRoles = new();

                    foreach (var roleInfo in RoleInfo.allRoleInfos)
                    {
                        var impostorCount = PlayerControl.AllPlayerControls.ToArray().ToList().Where(x => x.Data.Role.IsImpostor).Count();
                        ...
                        if (!CustomOptionHolder.draftModeChooseRoleNoTeam.getBool())
                        {
                        ...
                        {
                            var impMax = CustomOptionHolder.impostorRolesCountMax.getSelection();
                            var neutMax = CustomOptionHolder.neutralRolesCountMax.getSelection();
                            var crewMax = CustomOptionHolder.crewmateRolesCountMax.getSelection();

                            if (roleInfo.isImpostor)
                            {
                                if (alreadyPicked.Where(x => RoleInfo.roleInfoById[(RoleId)x].isImpostor).Count() >= impMax) continue;
                                if (alreadyPicked.Contains((byte)roleInfo.roleId)) continue;
                            }
                            else if (roleInfo.isNeutral)
                            {
                                if (alreadyPicked.Where(x => RoleInfo.roleInfoById[(RoleId)x].isNeutral).Count() >= neutMax) continue;
                                if (alreadyPicked.Contains((byte)roleInfo.roleId)) continue;
                            }
                    ...
                    if (GameObject.Find("RandomButton") == null && availableRoles.Count > 1)
                    {
                        ...
                        randomButton.transform.localPosition = new Vector3(
                            -8f + randomColumn * 5.5f,
                            -10 - randomRow * 3f - rowOffset * 3f,
                            0
                        );
                        ...
                        randomText.text = $"{"randomButtonText".Translate()}";
                ...
                __instance.TeamTitle.text =
                    $"{Helpers.ColorString(Color.red, $"<size=300%>{"welcomeText".Translate()}</size>")}\n\n\n<size=200%> {"currentlyPicking".Translate()}</size>\n\n\n<size=250%>{playerText}</size>";
    ...
    public static void receivePick(byte playerId, byte roleId)
    {
        ...
        if (CustomOptionHolder.draftModeChooseRoleNoTeam.getBool())
        {
            if (RoleInfo.allRoleInfos.First(x => (byte)x.roleId == roleId).isImpostor)
        ...
        try
        {
            ...
            var roleString = Helpers.ColorString(roleInfo.color, roleInfo.name);
            var roleLength =
            ...
            {
                roleString = Helpers.ColorString(Palette.ImpostorRed, "roleDraftImpRole".Translate());
                roleLength = "Impostor Role".Length;
            }
            else if (CustomOptionHolder.draftModeHideNeutralRoles.getBool() && roleInfo.isNeutral &&
                     !(playerId == PlayerControl.LocalPlayer.PlayerId))
            {
                roleString = Helpers.ColorString(Palette.Blue, "roleDraftNeutRole".Translate());
                roleLength = "Neutral Role".Length;
                     ...
                     !(playerId == PlayerControl.LocalPlayer.PlayerId))
            {
                roleString = Helpers.ColorString(Palette.Blue, "roleDraftCrewRole".Translate());
