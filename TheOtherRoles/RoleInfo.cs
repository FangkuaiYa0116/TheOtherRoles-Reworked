    ...
    public class RoleInfo
    {
            ...
            bool suppressGhostInfo = false)
        {
            ...
            roleName = string.Join(" ",
                getRoleInfoForPlayer(p, showModifier).Select(x => useColors ? Helpers.ColorString(x.color, x.name) : x.name)
                    .ToArray());
            ...
            if (HandleGuesser.isGuesserGm && HandleGuesser.isGuesser(p.PlayerId))
            {
                ...
                if ((!Helpers.isEvil(p) && playerCompleted < HandleGuesser.tasksToUnlock) || remainingShots == 0)
                    roleName += Helpers.ColorString(Color.gray, $" ({"guesser".Translate()})");
                else
                    roleName += Helpers.ColorString(Color.white, $" ({"guesser".Translate()})");
            }
        ...
        public static async Task loadReadme()
        {
            if (ReadmePage == "")
            {
                ...
                HttpResponseMessage response = await client.GetAsync(Helpers.isChinese() ? "https://rawgithub.fangkuai.fun/FangkuaiYa0116/TheOtherRoles/main/README.md" : "https://raw.githubusercontent.com/FangkuaiYa0116/TheOtherRoles/main/README.md");
